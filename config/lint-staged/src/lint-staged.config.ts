...
const config = (files) => {
  ...
  const filesPrettier = micromatch(
    files,
    micromatch.braces('**.{cjs,js,jsx,mjs,ts,tsx,html,json,md,mdx,css,scss,yml,yaml}', {
      expand: true,
    }),
  )
  if (filesPrettier.length) {
    ...
    isCI
      ...
      : lintStaged.push(`prettier --ignore-unknown --write ${filenames}`, `git add ${filenames} -u`)
  }
  ...
  const filesSyncpack = micromatch(files, micromatch.braces('**/package.json', { expand: true }))
