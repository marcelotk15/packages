...
const contributorsProhibitListDefault = {
  ...
  email: ['noreply@github.com', 'users.noreply.github.com', 'semantic-release-bot@martynus.net'],
}

const contributorsSubtitle = ['Props to', 'Kudos to', 'Thanks to', 'Brought to you by']
...
const contributor = async (context, commits, meta) => {
  ...
  contributorsProhibitListEmail.map((eject) => {
    const ejectIndex = _findIndex(authors, (author: any) => author.email.includes(eject))
    if (ejectIndex !== -1) authors.splice(ejectIndex)
  })
